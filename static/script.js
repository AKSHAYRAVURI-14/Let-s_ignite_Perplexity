document.addEventListener('DOMContentLoaded', () => {
    ...
    webBtn.addEventListener('click', () => {
        ...
        if (isWebSearchEnabled) {
            webBtn.classList.add('active-tool');
            userInput.placeholder = 'Search the live web or ask Ouro AI...';
        } else {
            webBtn.classList.remove('active-tool');
            userInput.placeholder = 'Ask anything, analyze documents, or explore ideas...';
        }
    });
    ...
    fileUpload.addEventListener('change', function() {
        if (this.files && this.files[0]) {
            ...
            if (file.type.startsWith('image/')) {
                ...
                filePreviewImage.style.background = 'var(--border-color)';
            } else {
    ...
    micBtn.addEventListener('click', async () => {
        if (!isRecording) {
            try {
                ...
                userInput.placeholder = "Listening crystalline voice input...";
            } catch (err) {
    ...
    async function handleUserInput() {
        ...
        try {
            ...
            if (file) {
                ...
                requestBody.append('browse_web', isWebSearchEnabled);
                // Do not set Content-Type for FormData, fetch does it automatically with boundary
            } else {
                // Use JSON if there is no file
                headers['Content-Type'] = 'application/json';
                requestBody = JSON.stringify({ message: message, chat_id: activeChatId, browse_web: isWebSearchEnabled });
            }

            // Show a browsing indicator before appending real message if needed
            const loadingId = 'loading-' + Date.now();
            if (isWebSearchEnabled) {
                appendMessage('ai', '<i class="ph ph-circle-notch ph-spin"></i> Browsing the web...', loadingId);
            }
            ...
            if (data.response) {
                ...
                // If there are sources appended, add citations block
    ...
    function appendMessage(role, text, id = null) {
        ...
        avatar.innerHTML = role === 'ai' ? '<i class="ph ph-sparkle"></i>' : 'N';
    ...
    function appendSources(sources) {
        ...
        sourceDiv.className = 'message ai sources-container';
        sourceDiv.style.marginTop = '-15px';
        
        let html = `<div style="display: flex; gap: 10px; flex-wrap: wrap; margin-left: 64px;">`;
        sources.forEach((url, i) => {
            const domain = new URL(url).hostname.replace('www.', '');
            html += `<a href="${url}" target="_blank" class="source-badge" title="${url}">
                        <i class="ph ph-link"></i> ${i+1}. ${domain}
                     </a>`;
        });
        html += `</div>`;
